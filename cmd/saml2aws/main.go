...
func main() {
	...
	cmdLogin.Flag("google-challenge", "Specific to GoogleApps, a prioritized list of challenge types used during login. This flag can be specified multiple times to set the order of challenges (e.g., --google-challenges=totp --google-challenges=dp).").HintOptions("totp", "ipp", "dp").EnumsVar(&loginFlags.GoogleChallenges, "totp", "ipp", "dp")
	cmdLogin.Flag("auto-fill", "Automatically fills prompts with stored values during login, skipping manual input.").BoolVar(&loginFlags.AutoFill)
